...
public class LogUtil {
	 ...
	 * 日志文件名称绝对路径
	 */
	private static final String LOGFILENAME = Environment.getExternalStorageDirectory().getPath() + "/.SecurityCenter_log";
	...
	public static final String LOGTAG = "SecurityCenter";
	 ...
	 * 
	 * @param msg
	 *            日志信息, 支持动态传参可以是一个或多个(避免日志信息的+操作过早的执行)
	 * @author kjxu
	 */
	...
	private static void writeLogToFile(int degree, String tag, String msg, Throwable tr) {
		if (IS_NEED_FILELOG) {
			...
			sb.append(df.format(Calendar.getInstance().getTime())).append(", <D>").append(degreeLabel.get(degree))
					.append(", <T>").append(tag).append(", <M>").append(msg);
				...
				if (null != stacks) {
					...
					for (int i = 0; i < stacks.length; i++) {
						sb.append("\t\tat ").append(stacks[i].getClassName()).append(".")
								.append(stacks[i].getMethodName()).append("(").append(stacks[i].getClassName())
								.append(".java").append(" ").append(stacks[i].getLineNumber()).append(")")
								.append("\r\n");
					}
				}
			}
